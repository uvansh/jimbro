...
export default function ProgressPage() {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        if (data.length > 0 &&
          (!latestData || data[data.length - 1]._id !== latestData._id)) {
          setLatestData(data[data.length - 1]);
        } else if (data.length === 0 && latestData !== null) {
          setLatestData(null);
        }
      } catch (error) {
        console.error(error);
  ...
  const resetDailyTracker = async () => {
    try {
      ...
      if (!res.ok) {
        console.log('Failed to reset daily tracker');
      }
      else{
        console.log('Daily tracker reset successfully');
      }
  ...
  useEffect(()=>{
    const checkForNewDay = async () => {
      ...
      if (!lastCreatedAt || now.getDate() !== lastCreatedAt.getDate()) {
        // Save previous day's data to historical collection
        // Reset daily tracker
        await resetDailyTracker();
      }
    }
    checkForNewDay();
  },[]);
        ...
        {latestData?._id ?
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {latestData?.updatedAt ? <p>Last updated at {latestData?.updatedAt.slice(11, 16)}</p> : "Not updated yet"}
              </CardDescription>
            </CardHeader>
            <CardContent>
