...
export default function ProgressPage() {
  ...
  useEffect(() => {
    ...
    fetchProgress();
  }, [isDataUpdated]); // This will re-run when isDataUpdated changes

  const resetDailyTracker = async () => {
  ...
  useEffect(() => {
    ...
    checkForNewDay();
  }, [initialLoadComplete, latestData?._id]); // Only run when initialLoadComplete changes or latestData changes
  ...
  return (
    <div>
      ...
      <button 
        ...
        className="mb-4 px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
      >
        {isResetting ? (
          <>
        ...
        {latestData?._id ?
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {latestData?.updatedAt && (latestData.targetCalories > 0 || latestData.targetExerciseTime > 0 || latestData.caloriesIntake > 0 || latestData.exerciseTime > 0) ? 
                  <p>Last updated at {new Date(latestData.updatedAt).toLocaleTimeString()}</p> 
                  : "Not updated yet"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <h1 className='font-semibold text-xl'>🥑Calories</h1>
              <Progress className="h-10 w-full" value={latestData?.caloriesIntake || 0} totalvalue={latestData?.targetCalories || 0} measure="cal" />
              <h1 className='font-semibold text-xl mt-2'>🏋️‍♂️Exercise</h1>
              <Progress className="h-10 w-full" value={latestData?.exerciseTime || 0} totalvalue={latestData?.targetExerciseTime || 0} measure="min" />
              <h1 className='font-semibold text-md text-green-500 mt-5'>
                {(latestData?.targetCalories !== 0 && latestData?.targetExerciseTime !== 0) &&
                  ...
                  ? "🎉Congratulations you have finished your target!" : (latestData?.targetCalories === 0 && latestData?.targetExerciseTime === 0) ? "🎯Set up a target now!" : "💪Keep Going!"}
              </h1>
            </CardContent>
          </Card> : ""}
